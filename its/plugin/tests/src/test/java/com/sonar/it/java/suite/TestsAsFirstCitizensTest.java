...
public class TestsAsFirstCitizensTest {
  ...
  public void test_and_main_sources_separated() throws Exception {
    ...
    List<Issue> issues = TestUtils.issuesForComponent(orchestrator, PROJECT_KEY_NORMAL);
    assertThat(issues).hasSize(3);

    String testFileKey = keyForTestFile(PROJECT_KEY_NORMAL);
    List<Issue> testFileIssues = issues.stream().filter(issue -> testFileKey.equals(issue.getComponent())).collect(Collectors.toList());
    assertThat(testFileIssues).hasSize(1);
    assertThat(testFileIssues.stream().map(Issue::getRule)).containsOnly("squid:S2970");
  }
  ...
  public void tests_as_first_citizens() throws Exception {
    ...
    List<Issue> issues = TestUtils.issuesForComponent(orchestrator, PROJECT_KEY_TESTS_AS_FIRST_CITIZENS);
    assertThat(issues).hasSize(5);

    String testFileKey = keyForTestFile(PROJECT_KEY_TESTS_AS_FIRST_CITIZENS);
    List<Issue> testFileIssues = issues.stream().filter(issue -> testFileKey.equals(issue.getComponent())).collect(Collectors.toList());
    assertThat(testFileIssues).hasSize(3);
    assertThat(testFileIssues.stream().map(Issue::getRule)).containsExactlyInAnyOrder("squid:S2970", "squid:S1172", "squid:S2259");
  }
