...
public class UnclosedResourcesCheck extends SECheck implements JavaFileScanner {
  ...
  private static class CloseableArgument {
    ...
    CloseableArgument(SymbolicValue value) {
      this.value = value;
    }
  }

  private static abstract class SingleTreeNodeVisitor extends BaseTreeVisitor {
  ...
  private static class PostStatementVisitor extends SingleTreeNodeVisitor {
    ...
    private final CloseableArgument closeableArgument;

    public PostStatementVisitor(CheckerContext context, CloseableArgument closeableArgument) {
      super(context.getState());
      constraintManager = context.getConstraintManager();
      this.closeableArgument = closeableArgument;
    }

    @Override
    public void visitNewClass(NewClassTree syntaxNode) {
      if (closeableArgument == null) {
        return;
      }
      final SymbolicValue argumentValue = closeableArgument.value;
      final SymbolicValue instanceValue;
      if (argumentValue == null) {
        instanceValue = programState.peekValue();
      } else {
        final Pop pop = programState.unstackValue(1);
        instanceValue = constraintManager.createWrappedSymbolicValue(argumentValue);
        programState = pop.state.stackValue(instanceValue);
      }
      programState = programState.addConstraint(instanceValue, NullConstraint.OPENED);
    }
  }

  private static class PreStatementVisitor extends SingleTreeNodeVisitor {

    private static final String CLOSE_METHOD_NAME = "close";

    CloseableArgument closeableArgument;
    ...
    public void visitNewClass(NewClassTree syntaxNode) {
      ...
      if (isOpeningResource(syntaxNode)) {
        closeableArgument = getCloseableArgument(syntaxNode, arguments);
      }
    }
    ...
    private static boolean isWithinTryHeader(NewClassTree syntaxNode) {
    ...
    private static CloseableArgument getCloseableArgument(NewClassTree syntaxNode, List<SymbolicValue> arguments) {
      ...
      for (ExpressionTree argument : syntaxNode.arguments()) {
        ...
        if (isCloseable(type)) {
          return new CloseableArgument(needsClosing(type) ? value : null);
        }
      }
      return new CloseableArgument(null);
    }

    private ProgramState closeResource(ProgramState programState, final SymbolicValue target) {
  ...
  // TODO This variable should be placed into the checker context to ensure
  // TODO that we always get the value corresponding to the previous post
  private CloseableArgument closeableArgument;
  ...
  public ProgramState checkPreStatement(CheckerContext context, Tree syntaxNode) {
    ...
    closeableArgument = visitor.closeableArgument;
    return visitor.programState;
  }

  @Override
  public ProgramState checkPostStatement(CheckerContext context, Tree syntaxNode) {
    final PostStatementVisitor visitor = new PostStatementVisitor(context, closeableArgument);
    syntaxNode.accept(visitor);
    return visitor.programState;
  }
  ...
  public void checkEndOfExecutionPath(CheckerContext context, ConstraintManager constraintManager) {
    final List<Tree> openedResources = constraintManager.getOpenedResources(context.getState());
