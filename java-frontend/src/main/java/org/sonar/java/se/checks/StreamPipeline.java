...
public class StreamPipeline extends SECheck {

  private StreamConsumedCheck streamConsumedCheck;

  public StreamPipeline(@Nullable StreamConsumedCheck streamConsumedCheck) {
    this.streamConsumedCheck = streamConsumedCheck;
  }
  ...
  private ProgramState handleMethodInvocation(CheckerContext context, Tree syntaxNode) {
    ...
    if ((isIntermediateOperation(mit) || isTerminalOperation(mit))
        ...
        && streamConsumedCheck != null) {
      streamConsumedCheck.reportIssue(syntaxNode, "Refactor this code so that this consumed stream pipeline is not reused.", flow(invocationTarget, context.getNode()));
      return null;
    }
