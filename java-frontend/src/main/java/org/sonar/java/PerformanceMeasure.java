  ...
  public void report(StringBuilder out, String parentContext) {
    ...
    if (totalDurationNanos != 0) {
      ...
      if (childrenDurationNanos != 0) {
        ...
        minusChildren = ", minus children duration " + LONG_FORMATTER.format(diffMicros) + " µs ";
      }
      out.append(context)
        .append(" ( ")
        .append(LONG_FORMATTER.format(totalDurationNanos / NANO_PER_MICRO))
        .append(" µs during ").append(LONG_FORMATTER.format(callsCount)).append(" call(s)").append(minusChildren).append(")\n");
    }
    if (childrenMap != null) {
