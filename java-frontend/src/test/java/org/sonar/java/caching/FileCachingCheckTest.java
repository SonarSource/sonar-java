...
class FileCachingCheckTest {
  ...
  @Test
  void restoreFromCache_restores_data_and_reports_a_hit() {
    when(readCache.readBytes(CACHE_KEY)).thenReturn("a,b".getBytes(StandardCharsets.UTF_8));

    assertThat(check.restoreFromCache(context(true))).isTrue();
    assertThat(check.moduleState.get(FILE_KEY)).containsExactly("a", "b");
  }
  ...
  @Test
  void restoreFromCache_replaces_the_previous_contribution_for_the_same_file() {
    when(readCache.readBytes(CACHE_KEY))
      .thenReturn("a".getBytes(StandardCharsets.UTF_8))
      .thenReturn("b".getBytes(StandardCharsets.UTF_8));

    assertThat(check.restoreFromCache(context(true))).isTrue();
    assertThat(check.restoreFromCache(context(true))).isTrue();

    assertThat(check.moduleState.get(FILE_KEY)).containsExactly("b");
  }
  ...
  private static class TestCachingCheck implements FileCachingCheck<List<String>> {
    ...
    private final Map<String, List<String>> moduleState = new HashMap<>();
    ...
    public void restore(InputFileScannerContext context, List<String> data) {
      moduleState.put(context.getInputFile().key(), data);
    }
  }
}
