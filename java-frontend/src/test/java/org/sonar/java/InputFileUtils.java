...
class InputFileUtils {
  public static InputFile addFile(Path temp, String code, SensorContextTester context) throws IOException {
    ...
    if (matcher.find()) {
      ...
      File file = java.nio.file.Files.createFile(temp.resolve(className + ".java")).toFile().getAbsoluteFile();
      return generateInputFile(code, context, file, type);
    } else {
      File file = java.nio.file.Files.createFile(temp.resolve("Unnamed.java")).toFile().getAbsoluteFile();
