...
public class UselessConditionCheck extends BaseTreeVisitor implements JavaFileScanner {
  ...
  private void checkCondition(ExpressionTree tree) {
    ExpressionEvaluatorVisitor results = new ExpressionEvaluatorVisitor(new ExecutionState(), tree);
    if (!results.falseStates.isEmpty() && results.trueStates.isEmpty()) {
      raiseIssue(tree, "false");
    }
    if (!results.trueStates.isEmpty() && results.falseStates.isEmpty()) {
