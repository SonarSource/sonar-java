...
public class CollectionMethodsWithLinearComplexityCheck extends IssuableSubscriptionVisitor {
  ...
  private static final String COW_ARRAY_LIST = "java.util.concurrent.CopyOnWriteArrayList";
  private static final String COW_ARRAY_SET = "java.util.concurrent.CopyOnWriteArraySet";
  ...
  static {
    ...
    MethodMatcher collectionContains = collectionMethodMatcher().name("contains").addParameter(JAVA_LANG_OBJECT);
    builder.put(collectionContains, ImmutableSet.of(ARRAY_LIST, LINKED_LIST, COW_ARRAY_LIST, COW_ARRAY_SET, CONCURRENT_LINKED_QUEUE, CONCURRENT_LINKED_DEQUE));
    ...
    builder.put(collectionAdd, ImmutableSet.of(COW_ARRAY_SET, COW_ARRAY_LIST));

    MethodMatcher collectionRemove = collectionMethodMatcher().name("remove").addParameter(JAVA_LANG_OBJECT);
    builder.put(collectionRemove, ImmutableSet.of(ARRAY_LIST, COW_ARRAY_SET, COW_ARRAY_LIST));
  ...
  private static boolean matchesActualType(Symbol invocationTarget, Set<String> actualTypes) {
    return actualTypes.contains(actualType(invocationTarget).fullyQualifiedName());
  }
  ...
  private static Type actualType(Symbol symbol) {
