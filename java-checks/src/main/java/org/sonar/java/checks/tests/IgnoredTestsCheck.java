...
public class IgnoredTestsCheck extends IssuableSubscriptionVisitor {
      ...
      getSilentlyIgnoredAnnotation(symbolMetadata, annotationName)
        .ifPresent(
          annotationTree -> {
            ...
            JavaFileScannerContext.Location annotationLocation = new JavaFileScannerContext.Location(shortName, annotationTree);
            context.reportIssue(this, methodTree.simpleName(), message, List.of(annotationLocation), null);
          }
        );
    }
  ...
  private static boolean isSilentlyIgnored(SymbolMetadata symbolMetadata, String fullyQualifiedName) {
    return !getSilentlyIgnoredAnnotation(symbolMetadata, fullyQualifiedName).isEmpty();
  }
