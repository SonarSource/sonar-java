...
@Rule(key = "S6485")
public class KnownCapacityHashMapsCheck extends IssuableSubscriptionVisitor {
  ...
  public void visitNode(Tree tree) {
    ...
    if (!context.getJavaVersion().isJava19Compatible()) {
      return;
    }

    NewClassTree newClassTree = (NewClassTree) tree;
    if (MATCHERS.matches(newClassTree)) {
      reportIssue(newClassTree, "Replace this call to the constructor with the better suited static method.");
    }
  }
