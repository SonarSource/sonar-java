...
public class SynchronizedOnConcurrentObjectCheck extends IssuableSubscriptionVisitor {
  ...
  private static final String CONCURRENT_PACKAGE_PREFIX = "java.util.concurrent.";
  ...
  public void visitNode(Tree tree) {
    ...
    if (isFromConcurrentPackage(expressionType)) {
  ...
  private static boolean isFromConcurrentPackage(Type type) {
    if (type.fullyQualifiedName().startsWith(CONCURRENT_PACKAGE_PREFIX)) {
      return true;
    }
    return type.symbol().superTypes().stream()
      .anyMatch(superType -> superType.fullyQualifiedName().startsWith(CONCURRENT_PACKAGE_PREFIX));
  }
