...
public class S3949Check extends IssuableSubscriptionVisitor {
  ...
  public void visitNode(Tree tree) {
    ...
    } else if (tree instanceof BinaryExpressionTree binary) {
      checkBinary(binary);
    } else {
      checkCompound((AssignmentExpressionTree) tree);
    }
  }

  private void checkBinary(BinaryExpressionTree tree) {
    if ((tree.is(Tree.Kind.MINUS) && isComparatorSubtraction(tree)) || isWidenedSink(tree)) {
      return;
    }
  ...
  private void checkNegation(UnaryExpressionTree tree) {
    ...
    if (operand == null || isMinimumValue(operand, tree.symbolType())) {
      return;
    }
  ...
  private void checkCompound(AssignmentExpressionTree tree) {
    Range variable = IntegerOverflowRange.rangeOf(tree.variable());
    Range value = IntegerOverflowRange.rangeOf(tree.expression());
    if (variable == null || value == null) {
      return;
    }
    Range result = switch (tree.kind()) {
      case PLUS_ASSIGNMENT -> variable.add(value);
      case MINUS_ASSIGNMENT -> variable.subtract(value);
      case MULTIPLY_ASSIGNMENT -> variable.multiply(value);
      default -> throw new IllegalStateException();
    };
    if (result.exceeds(tree.variable().symbolType())) {
      reportIssue(tree, MESSAGE);
    }
  }
  ...
  private static boolean isMinimumValue(Range range, Type type) {
    ...
    return range.low().equals(minimum) && range.high().equals(minimum);
  }

  private static boolean isExact(Range range) {
    return range.low().equals(range.high());
  }

  private static boolean isComparatorSubtraction(Tree tree) {
    Tree current = tree.parent();
    while (current != null && !current.is(Tree.Kind.CLASS, Tree.Kind.ENUM, Tree.Kind.INTERFACE, Tree.Kind.ANNOTATION_TYPE)) {
      if (current instanceof LambdaExpressionTree lambda && ComparisonMethodUtils.isComparatorLambda(lambda)) {
        return true;
      }
      if (current instanceof MethodTree method && ComparisonMethodUtils.isCompareMethod(method)) {
        return true;
      }
  ...
  private static boolean isWidenedSink(ExpressionTree expression) {
    ...
    if (parent instanceof TypeCastTree cast) {
      return isWider(cast.type().symbolType(), expression.symbolType());
    }
  ...
  private static boolean isWider(Type sink, Type source) {
    return source.isPrimitive(Type.Primitives.INT)
      && (sink.isPrimitive(Type.Primitives.LONG) || sink.isPrimitive(Type.Primitives.FLOAT) || sink.isPrimitive(Type.Primitives.DOUBLE));
  }
