...
abstract class A {
  public void foo() {
    ...
    foo((a, b) -> {return a.length();}); // Noncompliant {{Specify a type for: 'a', 'b'}}
//       ^^^^
    foo(s -> {return s.length();}); // Noncompliant {{Specify a type for: 's'}}
//      ^
    foo((a, b, c) -> a.length()); // Noncompliant {{Specify a type for: 'a', 'b', 'c'}}
//       ^^^^^^^
    foo((a, b, c) -> {return a.length();}); // Noncompliant {{Specify a type for: 'a', 'b', 'c'}}
//       ^^^^^^^
  }
