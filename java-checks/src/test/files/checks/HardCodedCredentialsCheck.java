...
class A {
  ...
  private void a(char[] pwd, String var) {
    ...
    passphrase = PASSED.toCharArray(); // Noncompliant, previously FN
    ...
    if(PASSED.equals(password)) { // Noncompliant, previously FN
    }
    if(password.equals("")) {
    }
    ...
    conn = DriverManager.getConnection("jdbc:mysql://xxx/", "root", PASSED); // Noncompliant, previously FN
