...
public class MathClampMethodsCheckSampleWithoutSemantic {

  public void testNoncompliant(int min, int max, int value) {
    int clampedValueConditional1 = value > max ? max : value < min ? min : value; // Noncompliant
    ...
    int clampedValueMethodInvocation1 = Math.min(min, Math.max(max, value)); // Noncompliant
