...
class ToArrayCheckSample<T> {
  <E extends T> Object[] foo(List<String> listOfString, List<String> listOfNumber, Set rawSet, Collection<E> col) {
    String[] a1 = (String[]) listOfString.toArray(); // Noncompliant [[quickfixes=qf1]] {{Pass "new String[0]" as argument to "toArray".}}
//                           ^^^^^^^^^^^^^^^^^^^^^^
    // fix@qf1 {{Pass "new String[0]" as argument}}
    // edit@qf1 [[sc=51;ec=51]] {{new String[0]}}
    // edit@qf1 [[sc=19;ec=30]] {{}}
    Number[] a21 = (Number[])       listOfNumber.toArray(); // Noncompliant [[quickfixes=qf2]] {{Pass "new Number[0]" as argument to "toArray".}}
//                                  ^^^^^^^^^^^^^^^^^^^^^^
    // fix@qf2 {{Pass "new Number[0]" as argument}}
    // edit@qf2 [[sc=58;ec=58]] {{new Number[0]}}
    // edit@qf2 [[sc=20;ec=37]] {{}}
    Number[] a22 = (Number[]) rawSet.toArray(); // Noncompliant [[quickfixes=qf3]] {{Pass "new Number[0]" as argument to "toArray".}}
//                            ^^^^^^^^^^^^^^^^
    ...
    Number[] a23 = (Number[]) col.toArray(); // Noncompliant [[quickfixes=qf4]] {{Pass "new Number[0]" as argument to "toArray".}}
//                            ^^^^^^^^^^^^^
