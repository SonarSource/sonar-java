...
public class StaticFieldInjectionNotSupportedCheckSample {
  ...
  public class StaticInjectComponent {
    ...
    @javax.inject.Inject // Noncompliant {{Remove the "@Inject" annotation targeting the static field.}}
  //^^^^^^^^^^^^^^^^^^^^
    ...
    @Inject // Noncompliant {{Remove the "@Inject" annotation targeting the static method.}}
  //^^^^^^^
    ...
    static void staticValueParameter(@Value("aValue") String s){} // Noncompliant  {{Remove the "@Inject" annotation targeting the parameter.}}
  //                            ^^^^^^^^^^^^^^^^
