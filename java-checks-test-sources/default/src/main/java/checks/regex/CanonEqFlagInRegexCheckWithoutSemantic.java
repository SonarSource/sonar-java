...
public class CanonEqFlagInRegexCheckWithoutSemantic {
  ...
  @jakarta.validation.constraints.Email(regexp = "éeéc") // Noncompliant

  String email3;

  @jakarta.validation.constraints.Email(regexp = "éeéc", flags = jakarta.validation.constraints.Pattern.Flag.CANON_EQ) // Noncompliant
  String email4;

  void noncompliant(String str) {
    Pattern.compile("éeéc"); // Noncompliant
  ...
  void can_not_set_flag_directly() {
    "éeé".replaceAll("é" , "e"); // Noncompliant
    "éeé".replaceFirst("é" , "e"); // Noncompliant
    "éeé".matches("é"); // Noncompliant
    Pattern.matches("é", "input"); // Noncompliant
  }
