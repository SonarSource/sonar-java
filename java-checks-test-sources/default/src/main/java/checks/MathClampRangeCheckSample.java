...
public class MathClampRangeCheckSample {
  void test(
    ...
    // equals
    Math.clamp(d1, d2, d2); // Noncompliant [[sc=20;ec=22;quickfixes=!]] {{Change the "clamp(value,min,max)"'s arguments so "min" is not equals to "max".}}
    ...
    Math.clamp(l1, 10L, 0L); // Noncompliant [[sc=25;ec=27;secondary=+0;quickfixes=qf1]] {{Change the "clamp(value,min,max)"'s arguments so "max" is not always less than "min".}}
