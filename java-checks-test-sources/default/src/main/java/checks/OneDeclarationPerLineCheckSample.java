...
//             ^
  ...
  int a; int b; int c; int d; // Noncompliant {{Declare "b" and all following declarations on a separate line.}}
//           ^
//  ^^^<
  int e;

  private int j1 = -1, j2 // Noncompliant {{Declare "j2" and all following declarations on a separate line.}}
//                     ^^
...
//  ^^^<
  ...
  private int i1 = -1, i2, i3 = 1; // Noncompliant {{Declare "i2" and all following declarations on a separate line.}}
//                     ^^
//  ^^^<
  ...
  void mixedAreReportedTogether() {
    int k; int l; int m, n; // Noncompliant [[quickfixes=qf_mixed1]]
//             ^
//  ^^^<
    ...
    int k1; int l1, m1; int n1; // Noncompliant [[quickfixes=qf_mixed2]]
//              ^^
//  ^^^<
  ...
  class TestQuickFix {
    int i; int j; // Noncompliant [[quickfixes=qf1]]
//             ^
    // fix@qf1 {{Declare on separated lines}}
    // edit@qf1 [[sc=11;ec=12]] {{\n    }}

    int i2; int j2;     int k2;    int l2; // Noncompliant [[quickfixes=qf2]]
//              ^^
    ...
    int i3, j3; // Noncompliant [[quickfixes=qf3]]
//          ^^
    // fix@qf3 {{Declare on separated lines}}
    // edit@qf3 [[sc=11;ec=13]] {{;\n    int }}

    int i4, j4, k4, l4; // Noncompliant [[quickfixes=qf4]]
//          ^^
    ...
    final int i5 = 1, j5 = 1; // Noncompliant [[quickfixes=qf5]]
//                    ^^
    ...
    j6, // Noncompliant [[quickfixes=qf6]]
//  ^^
    ...
    void m() {
      int i7; m(); int j7; // Noncompliant [[quickfixes=qf17]]
//                     ^^
  ...
  int correct; int indentation; // Noncompliant [[quickfixes=qf_indentation]]
//                 ^^^^^^^^^^^
